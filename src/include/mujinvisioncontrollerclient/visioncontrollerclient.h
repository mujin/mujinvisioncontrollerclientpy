...
public:
    ...
    /// \brief Send a command to sensorbridge and receive the result
    void SendAndReceiveCommand(const std::string& command,
                               const rapidjson::Value& rParameterValue,
                               rapidjson::Value& rReturnValue,
                               rapidjson::Document::AllocatorType& rReturnAlloc,
                               double timeout);

    void SendAndReceiveConfig(const std::string& command,
                              const rapidjson::Value& rParameterValue,
                              rapidjson::Value& rReturnValue,
                              rapidjson::Document::AllocatorType& rReturnAlloc,
                              double timeout);
...
private:
    ...
    void _SendCommand(zmq::message_t& message, const char* commandString, uint64_t timeoutMS);

    /// @brief Receive response.
    void _ReceiveResponse(zmq::message_t& message, const char* commandString, uint64_t timeoutMS);

    /// @brief Send JSON command.
    void _SendCommandJSON(rapidjson::Value& rCommand, rapidjson::Document::AllocatorType& rAlloc, const char* commandString, uint64_t timeoutMS);

    /// @brief Receive and deserialize JSON response in rapidjson::Value.
    void _ReceiveResponseJSON(rapidjson::Document& rResponse, const char* commandString, uint64_t timeoutMS);
    ...
    boost::shared_ptr<mujincontrollercommon::ZmqSocketPool> _commandSocketPool; ///< pool to create sockets
    boost::shared_ptr<zmq::socket_t> _currentCommandSocket;                     ///< current created socket

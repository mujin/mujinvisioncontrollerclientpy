...
void VisionControllerClient::SendAndReceiveCommand(const std::string& command, const rapidjson::Value& rParameterValue, rapidjson::Value& rReturnValue, rapidjson::Document::AllocatorType& rReturnAlloc, double timeout)
{
    _rAlloc->Clear();
    ...
    _SendCommandJSON(rCommand, rCommand.GetAllocator(), command.c_str(), timeoutMS);

    rapidjson::Document rResponse(_rAlloc.get());
    const uint64_t timeSpentSendingMS = _GetMilliTime64() - startTimeMS;
    _ReceiveResponseJSON(rResponse, command.c_str(), timeoutMS-timeSpentSendingMS);

    rReturnValue.CopyFrom(rResponse, rReturnAlloc);
}

void VisionControllerClient::_SendCommandJSON(rapidjson::Value& rCommand, rapidjson::Document::AllocatorType& rAlloc, const char* commandString, uint64_t timeoutMS)
{
    ...
    _SendCommand(message, commandString, timeoutMS);
}

void VisionControllerClient::_SendCommand(zmq::message_t& message, const char* commandString, uint64_t timeoutMS)
{
    ...
    while (_GetMilliTime64() - startTime < timeoutMS) {
        ...
        try {
            _currentCommandSocket->send(message, zmq::send_flags::none);
            return;
        }
            ...
            if (!recreatedOnce) {
                ...
                if (!!_currentCommandSocket) {
                    _currentCommandSocket->close();
                    _currentCommandSocket.reset();
                }
                recreatedOnce = true;
            }
...
void VisionControllerClient::_ReceiveResponse(zmq::message_t& message, const char* commandString, uint64_t timeoutMS)
{
    const uint64_t startTime = _GetMilliTime64();
    while ((_GetMilliTime64() - startTime < timeoutMS)) {
        try {
            zmq::pollitem_t pollitems[] = {
                { (void*)*_currentCommandSocket, 0, ZMQ_POLLIN, 0 },
            };
            zmq::poll(pollitems, 1, std::chrono::milliseconds{50}); // wait ms for message
            if (pollitems[0].revents & ZMQ_POLLIN) {
                zmq::recv_result_t success = _currentCommandSocket->recv(message, zmq::recv_flags::dontwait);
                if (!success) {
...
void VisionControllerClient::_ReceiveResponseJSON(rapidjson::Document& rResponse, const char* commandString, uint64_t timeoutMS)
{
    zmq::message_t message;
    _ReceiveResponse(message, commandString, timeoutMS);
...
void VisionControllerClient::_EnsureSocket()
{
    if (!!_currentCommandSocket) {
        return; // already has a socket
    }

    if (!_commandSocketPool) {
        const std::string endpoint = boost::str(boost::format("tcp://%s:%d")%_host%(_port + 1));
        const int maxNumSockets = 2; // need at least 2 when destructing old socket
        _commandSocketPool = boost::make_shared<mujincontrollercommon::ZmqSocketPool>(_context, endpoint, maxNumSockets, 0.001 * _timeoutMS);
        MUJIN_LOG_DEBUG_FORMAT("initialized command socket pool to connect to \"%s\", caller id \"%s\"", endpoint%_callerId);
    }
    _currentCommandSocket = _commandSocketPool->AcquireSocket();
}
